// app/futebol/page.tsx
import Image from "next/image";
import styles from "@/app/futebol/page.module.css";
import { produtosFutebol } from '@/app/futebol/produtosFutebol';
import Layout from "../layoutGlobal/layoutGlobal";

export default function Futebol() {
  return (
  
   <Layout>
      <div className={styles.container}>
        <h1 className={styles.title}>Produtos de Futebol</h1>

        <div className={styles.produtosContainer}>
          {produtosFutebol.map((produto) => (
            <div key={produto.id} className={styles.card}>
              <Image
              src={`/images/banners/slide${produto.id}.jpg`}
              alt={produto.nome}
              width={300}
              height={200}
              className={styles.imagemProduto}
              />
              <div className={styles.cardBody}>
              <h2 className={styles.nomeProduto}>{produto.nome}</h2>
              <p className={styles.descricaoProduto}>{produto.descricao}</p>
              <p className={styles.precoProduto}>{produto.preco}</p>
              <button className={styles.botaoAdicionar}>Adicionar ao Carrinho</button>
              </div>
            </div>
          ))}
        </div>
      </div>
      </Layout>
   
  );
}
